...
@Stable
@Local
public class Fusion extends DefaultAction {

    private static final Logger LOG = LoggerFactory
            .getLogger(Fusion.class);
    ...
    private List<String> getEmbeddingsResponse(MavenQuery query) {
        ...
        try {
            ...
            List<String> solrIds = StreamSupport.stream(implementations.spliterator(), false)
            .map(node -> node.get("solr_id").asText())
            .collect(Collectors.toList());

            LOG.info("Obtained the following IDs via Embedding Search: " + solrIds);
            return solrIds;
        }
        catch (Throwable e) {
...
//                }
                ...
                try {
                    ...
                    if(!(queryModel instanceof MavenQuery)) {
                        ...
                        queryResult.setImplementations(filteredImplementations);
                        queryResult.setNumFound(filteredImplementations.size());
                        LOG.info("Number of results after embedding filter: " + queryResult.getNumFound());
                    }
    ...
    private List<System> retrieveImplementations(LSLExecutionContext context, DataSource dataSource, List<String> implementations) {
        ...
        ForkJoinTask<List<CodeUnit>> task = customThreadPool.submit(
                () -> implementations.stream().parallel().map(implId -> {
                    ...
                    LOG.info("Searching for Implementation ID", implId);
